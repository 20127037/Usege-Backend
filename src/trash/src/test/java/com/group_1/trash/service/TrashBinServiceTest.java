...
import com.group_1.master.service.TrashBinServiceImpl;
...
public class TrashBinServiceTest {
    ...
    private final com.group_1.master.service.TrashBinService getTrashBinService = new TrashBinServiceImpl(userDeletedFileRepository);
    private final com.group_1.trash.service.TrashBinService updateTrashBinService =
            new com.group_1.trash.service.TrashBinServiceImpl(userFileRepository, userRepository, userDeletedFileRepository,
                    userFilesInAlbumRepository,
                    new RabbitMQMessageProducer(getAmqpTemplate()),
                    new RabbitMQConfig("", "", "", "", ""));
    ...
    void testDeleteFileIncreaseImgCountDecreaseDelImgCount() {
        ...
        List<UserFile> delFiles = updateTrashBinService.createDeletedFiles(userId, uploadedFiles.subList(0, delCount).stream().map(UserFile::getFileName).toArray(String[]::new));
    ...
    void testDeleteAllFilesThenQuery() {
        UserInfo before = userRepository.getRecordByKey(DynamoDbRepository.getKey(userId));
        updateTrashBinService.createDeletedFilesFromAllFiles(userId);
    ...
    void testRestoreFiles() {
        ...
        updateTrashBinService.createDeletedFilesFromAllFiles(userId);
        UserInfo before = userRepository.getRecordByKey(DynamoDbRepository.getKey(userId));
        List<UserFile> resFiles = updateTrashBinService.restoreFiles(userId, uploadedFiles.subList(0, resCount).stream().map(UserFile::getFileName).toArray(String[]::new));
    ...
    void testRestoreAllFiles() {
        updateTrashBinService.createDeletedFilesFromAllFiles(userId);
        UserInfo before = userRepository.getRecordByKey(DynamoDbRepository.getKey(userId));
        List<UserFile> resFiles = updateTrashBinService.restoreAll(userId);
...
//        updateTrashBinService.createDeletedFiles(userId, uploadedFiles.subList(0, delCount).stream().map(UserFile::getFileName).toArray(String[]::new));
...
//        albumService.updateTrashBinService(userId, ALBUM, uploadedFiles.subList(0, firstAdd).stream().map(UserFile::getFileName).toArray(String[]::new));
//        albumService.updateTrashBinService(userId, TO_ALBUM, uploadedFiles.subList(firstAdd, uploadedFiles.size()).stream().map(UserFile::getFileName).toArray(String[]::new));
//
//        updateTrashBinService.createDeletedFilesFromAllFiles(userId);
//
...
//
//        updateTrashBinService.restoreAll(userId);
//
...
//    }
    ...
    int testQueryTrashWithNPerPage(int perPage)
    {
        ...
        do {
            QueryResponse<UserFile> queryResponse = getTrashBinService.queryImages(userId, perPage, RequestMapperUtils.mapPagingKey(prevKey));
