...
public class FileServiceTest {
    ...
    void testGetFileByNameWithAllFiles()
    {
        ...
        {
            UserFile file = fileService.getFileByName(userId, userFile.getFileName(), false);
    ...
    {
        int favCount = 3;
        QueryResponse<UserFile> queryResponse = fileService.queryFiles(userId, testImgPaths.length,
                false, null, null);
        assertThat(queryResponse.getResponse().size(), Matchers.equalTo(testImgPaths.length));
        for (int i = 0; i < favCount; i++)
        {
            fileService.updateFile(userId, UserFile.builder()
                    .fileName(uploadedFiles.get(i).getFileName())
                    .isFavourite(true).build());
        }
        queryResponse = fileService.queryFiles(userId, testImgPaths.length,
                false, null, null);
    ...
    void testUpdateAllPossibleFieldsOfFile()
    {
        ...
        UserFile update = UserFile.builder()
                ...
                .location(loc)
                .build();
        UserFile updated = fileService.updateFile(userId, update);
