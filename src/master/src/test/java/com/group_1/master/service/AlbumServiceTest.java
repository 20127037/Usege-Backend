...
public class AlbumServiceTest {
    ...
    void testCreateAlumIncreaseAlbumCount()
    {
        ...
        UserAlbum userAlbum = albumService.createAlbum(userId, ALBUM);
...
//    QueryFilesInAlbumResponse queryImages(String userId, String albumName, int limit, Map<String, AttributeValue> startKey);
    ...
    {
        List<UserFileInAlbum> fileInAlbums = albumService.addImagesToAlbum(userId, ALBUM, "dump file");
    ...
    void testAddFilesToAlbumIncreaseFileCountAndQuery()
    {
        ...
        List<UserFileInAlbum> fileInAlbums = albumService.addImagesToAlbum(userId, ALBUM, uploadedFiles.stream().map(UserFile::getFileName).toArray(String[]::new));
    ...
    {
        int delCount = 3;
        List<UserFileInAlbum> addedFiles = albumService.addImagesToAlbum(userId, ALBUM, uploadedFiles.stream().map(UserFile::getFileName).toArray(String[]::new));
        UserAlbum beforeAlbum = userAlbumRepository.getRecordByKey(DynamoDbRepository.getKey(userId, ALBUM));
        List<UserFileInAlbum> deletedFiles = albumService.deleteImagesFromAlbum(userId, ALBUM, addedFiles.subList(0, delCount).stream().map(UserFileInAlbum::getFileName).toArray(String[]::new));
    ...
    void testAddFilesThenMoveToAnotherAlbumAndQuery()
    {
        ...
        List<UserFileInAlbum> addedFiles = albumService.addImagesToAlbum(userId, ALBUM, uploadedFiles.stream().map(UserFile::getFileName).toArray(String[]::new));
        UserAlbum beforeAlbum = userAlbumRepository.getRecordByKey(DynamoDbRepository.getKey(userId, ALBUM));
        List<UserFileInAlbum> movedFiles = albumService.moveImages(userId, ALBUM, TO_ALBUM,
